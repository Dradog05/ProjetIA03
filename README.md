...
### Étape 1 — Lancer chaque Machine A
Sur chaque machine webcam :
```bash
python webcam_stream.py
```
Le terminal affiche l'adresse à noter, du type :
```
Flux disponible sur : http://192.168.x.x:5000/video_feed
```

### Étape 2 — Lancer la Machine B
Une option `--camera "NOM=URL"` par caméra (noms sans espace).

⚠️ Sous PowerShell (Windows), le `\` de continuation de ligne ne fonctionne
pas comme sous Linux/Mac — écris la commande sur **une seule ligne** :
```powershell
python person_detection.py --camera "Entree=http://<IP_MACHINE_A1>:5000/video_feed" --camera "Salle=http://<IP_MACHINE_A2>:5000/video_feed" --camera "Telephone=http://<IP_TELEPHONE>:8080/video"
```
Avec une seule caméra, une seule option `--camera` suffit.

Options disponibles :
| Option | Rôle | Défaut |
|---|---|---|
| `--camera NOM=URL` | Caméra à suivre (répétable) | obligatoire |
...
### Étape 3 — Consulter depuis un client
Depuis n'importe quel appareil du même réseau :
```
http://<IP_MACHINE_B>:5001/
```

### Utiliser un téléphone comme caméra (au lieu d'un PC)
Aucune modification de code n'est nécessaire : `--camera` accepte n'importe
quelle URL qui diffuse un flux MJPEG, y compris celle d'une application de
caméra IP installée sur un téléphone (ex. **IP Webcam** sur Android, gratuite).

1. Installer l'app, connecter le téléphone au même hotspot, démarrer le
   serveur dans l'app.
2. Noter l'adresse affichée (ex. `10.174.254.68:8080`) et tester le flux
   brut dans un navigateur : `http://<IP_TELEPHONE>:8080/video`.
3. Utiliser cette URL comme n'importe quelle autre caméra dans `--camera`.

**Cas particulier testé et validé** : le téléphone qui sert de caméra peut
être **le même téléphone que celui qui fait office de hotspot**. Il garde
une adresse IP sur son propre réseau (celle utilisée comme passerelle par
les autres machines), et une app comme IP Webcam peut y écouter directement
— pas besoin d'un téléphone dédié uniquement à la caméra.
...
Pistes d'évolution détaillées dans le rapport de projet
(`documentation_latex/`) : suivi/comptage d'entrées-sorties (tracking),
zones d'intérêt et alertes, historique et statistiques, serveur de
production, protection par mot de passe.
